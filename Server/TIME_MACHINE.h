...
namespace X {
    ...
    inline void split_by_ret(symb & block, vector<symb> &vs){
        splitf(block, "\n", vs);
    }
    ...
    public:
        ...
        //if something bad happened, toggle this.
        time_piece(symb block, closure piece, string category) :\
                 m__block(block),m__category(category),m__piece(piece)
        {
            set_seq(global_seq);
            global_seq++;
        }
    ...
    public:
      ...
      static std::stringstream output;
        ...
        long get_capsule_size();

        timefrag & get_fragment_by_index(int index);
        timefrag & get_fragment_by_time(long time) ;

        void insert_time_piece_behind(long fragment_time, time_piece tp, long seq);

        void delete_time_piece(long seq);

        void delete_time_fragment(long fragment_time);

        inline capsule & get_capsule(){return this->time_capsule;}

        void load_and_compile();

        void instant_stream_compile(std::stringstream & ss, bool runp);
        ...
        inline bool frozenp(){return frozen;}
        ...
        static void init_capsule(){
            time_capsule=vector<timefrag>();
        }

        inline long get_global_time(){
            update_time();
            return global_time;}
