    ...
    for (auto word : v){
        ...
        if (__this->dh.well_sperated_by_colonp(word)) {
            ...
            switch (the_slot_num) {
                ...
                cout<<"the property is not perfectly seperated by colon."<<endl;
            }
        }
...
void TIME_MACHINE::compile_definition_block(symb blk, string domain, bool instantp) {
    cout<<"Xompiler: Compiling definition block of a "<<domain<<"."<<endl;
    ...
    cout<<"Xompiler: Compile finished."<<endl;
}
...
void TIME_MACHINE::compile_do_block(symb blk, bool instantp) {
    cout<<"Xompiler: Compiling do block."<<blk<<endl;
    auto original_block=blk;
    symbv lex;
    symbv sublex;
    ...
    try {
        ...
        cout<<"Xompiler: "<<s.describe()<<endl;
        cout<<s.word_hint()<<endl;
        this->error_num+=1;
    }
    cout<<"Xcompiler: Compile finished."<<endl;
}
...
void ::X::TIME_MACHINE::load_and_compile(){
    vector<string> vs;
    std::ifstream ifs;
    ifs.open("/Users/Heranort/test.x");
    if(not ifs.is_open()){
    ...
    cout<<"Compilation tasks finished with "<<this->error_num<<" errors."<<endl;
    this->error_num=0; //resume it.
}

void TIME_MACHINE::dump_to_xfile() {
    std::ofstream ofs;
    ofs.open("/Users/Heranort/test.y");
    if(not ofs.is_open()){
        cout<<"cannot open file"<<endl;
        return;
    }
    for(auto & the_pair : karacter::lb_binding->get_lessons()){
        auto the_lesson=the_pair.second;
        ofs<<"[lesson]\n";
...
void TIME_MACHINE::save_current_time(time_piece cap) {
    ...
    cout<<"saving the world of time: "<<time_ago<<endl;
...
void TIME_MACHINE::travel_to_time(time_t curtime) {
    ...
    cout<<"TIME MACHINE: Traveling to time: "<<curtime<<endl;
    for (auto the_time_frag : this->time_capsule){
        if(the_time_frag.first<=curtime){
            cout<<"TIME MACHINE: Found time fragment "<<the_time_frag.first<<endl;
            auto the_timev=the_time_frag.second;
            for(auto piece : the_timev){
                cout<<"evolving piece "<<piece.get_seq()<<endl;
                piece();
                cout<<"TIME MACHINE: Successfully recovered time piece."<<endl;
            }
        }

    }
    cout<<"TIME MACHINE: Moriturus te saluto."<<endl;
}
